...
构建 **Control 面板** 单体镜像：后端用 `tsx` 直跑 `src/index.ts`（stateless panel，入口 `src/panel/`）。纯 API 服务（`/api/v1`、`/health`），不托管前端静态资源。
...
| `runtime` | 拷贝全仓源码、`npm install`，启动 Control |
...
实例表字段见 [`config/metadata-instances.README.md`](../config/metadata-instances.README.md)。若 Gateway 跑在宿主机，挂载的 `metadata-instances.json` 里 `gateway_endpoint` 须用容器可访问的地址（如 `http://host.docker.internal:8420`），不要用 `127.0.0.1`。
